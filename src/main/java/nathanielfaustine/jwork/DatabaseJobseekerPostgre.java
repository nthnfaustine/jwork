...
public class DatabaseJobseekerPostgre {
    ...
    public static Jobseeker addJobseeker (Jobseeker jobseeker) {
        ...
        try {
            ...
            String query =  "INSERT INTO database_jobseeker (id, name, email, password, date)"
    ...
    public static int getLastId() {
        ...
        try {
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT MAX (id) FROM database_jobseeker;");
            while (rs.next()) {
                value = rs.getInt("max");
            }
            rs.close();
    ...
    public static Jobseeker getJobseekerById(int id) {
        ...
        try {
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM database_jobseeker WHERE id = "+ id +"");
            while (rs.next()) {
                id = rs.getInt("id");
    ...
    public static Jobseeker getJobseekerLogin(String emailInput, String passwordInput) {
        ...
        try {
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM database_jobseeker WHERE email = '"+ emailInput +"' AND password = '"+ passwordInput + "';");
            while (rs.next()) {
                int id = rs.getInt("id");
                String name = rs.getString("name");
                String email = rs.getString("email");
    ...
    public static ArrayList<Jobseeker> getJobseekerDatabase() {
        ...
        try {
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM database_jobseeker;");
            while (rs.next()) {
                int id = rs.getInt("id");
                String name = rs.getString("name");
                String email = rs.getString("email");
